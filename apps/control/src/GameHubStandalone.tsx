...
export function GameHubStandalone() {
  ...
  const patch = useCallback(async <K extends keyof AppConfig>(section: K, value: Partial<AppConfig[K]>, success?: string) => {
  ...
  return <section className="game-hub-overlay" aria-label="Kho game LIVE">
    {loading ? <div className="game-hub-loading"><span/><strong>Đang mở kho game…</strong></div> : <GameHubScreen config={config} patch={patch} notify={notify}/>} 
