...
import { CSM_ASSERT, CubismLogDebug, CubismLogError, CubismLogWarning } from '../utils/cubismdebug.js';
import { ACubismMotion } from './acubismmotion.js';
import { CubismMotionCurve, CubismMotionCurveTarget, CubismMotionData, CubismMotionEvent, CubismMotionPoint, CubismMotionSegment, CubismMotionSegmentType } from './cubismmotioninternal.js';
...
/**
 * Cubism SDK R2 ä»¥å‰ã®ãƒ¢ãƒ¼ã‚·ãƒ§ãƒ³ã‚’å†ç¾ã•ã›ã‚‹ãªã‚‰ true ã€ã‚¢ãƒ‹ãƒ¡ãƒ¼ã‚¿ã®ãƒ¢ãƒ¼ã‚·ãƒ§ãƒ³ã‚’æ­£ã—ãå†ç¾ã™ã‚‹ãªã‚‰ false ã€‚
 */
...
export class CubismMotion extends ACubismMotion {
    ...
    static create(buffer, size, onFinishedMotionHandler, onBeganMotionHandler, shouldCheckMotionConsistency = false) {
        ...
        // NOTE: Editorã§ã¯ãƒ«ãƒ¼ãƒ—ã‚ã‚Šã®ãƒ¢ãƒ¼ã‚·ãƒ§ãƒ³æ›¸ãå‡ºã—ã¯éžå¯¾å¿œ
    ...
    doUpdateParameters(model, userTimeSeconds, fadeWeight, motionQueueEntry) {
        ...
        if (timeOffsetSeconds < 0.0) {
            timeOffsetSeconds = 0.0; // ã‚¨ãƒ©ãƒ¼å›žé¿
        }
        ...
        let lipSyncFlags = 0;
        let eyeBlinkFlags = 0;
        //çž¬ãã€ãƒªãƒƒãƒ—ã‚·ãƒ³ã‚¯ã®ã‚¿ãƒ¼ã‚²ãƒƒãƒˆæ•°ãŒä¸Šé™ã‚’è¶…ãˆã¦ã„ã‚‹å ´åˆ
    ...
    /**
     * è‡ªå‹•ã‚¨ãƒ•ã‚§ã‚¯ãƒˆãŒã‹ã‹ã£ã¦ã„ã‚‹ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿IDãƒªã‚¹ãƒˆã®è¨­å®š
     * @param eyeBlinkParameterIds    è‡ªå‹•ã¾ã°ãŸããŒã‹ã‹ã£ã¦ã„ã‚‹ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿IDã®ãƒªã‚¹ãƒˆ
     * @param lipSyncParameterIds     ãƒªãƒƒãƒ—ã‚·ãƒ³ã‚¯ãŒã‹ã‹ã£ã¦ã„ã‚‹ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿IDã®ãƒªã‚¹ãƒˆ
     */
    ...
    constructor() {
        ...
        this._isLoopFadeIn = true; // ãƒ«ãƒ¼ãƒ—æ™‚ã«ãƒ•ã‚§ãƒ¼ãƒ‰ã‚¤ãƒ³ãŒæœ‰åŠ¹ã‹ã©ã†ã‹ã®ãƒ•ãƒ©ã‚°
    ...
    updateForNextLoop(motionQueueEntry, userTimeSeconds, time) {
        switch (this._motionBehavior) {
            ...
            default:
                motionQueueEntry.setStartTime(userTimeSeconds - time); // æœ€åˆã®çŠ¶æ…‹ã¸
     ...
     *
     * @param motionJson  motion3.jsonãŒèª­ã¿è¾¼ã¾ã‚Œã¦ã„ã‚‹ãƒãƒƒãƒ•ã‚¡
     * @param size        ãƒãƒƒãƒ•ã‚¡ã®ã‚µã‚¤ã‚º
     * @param shouldCheckMotionConsistency motion3.jsonæ•´åˆæ€§ãƒã‚§ãƒƒã‚¯ã™ã‚‹ã‹ã©ã†ã‹
     */
    ...
    /**
     * ç¾åœ¨æ™‚é–“ã®é€æ˜Žåº¦ã®å€¤ã‚’è¿”ã™
     *
     * @return success:ãƒ¢ãƒ¼ã‚·ãƒ§ãƒ³ã®å½“è©²æ™‚é–“ã«ãŠã‘ã‚‹Opacityã®å€¤
     */
    getModelOpacityValue() {
        return this._modelOpacity;
    }
    /**
     * ãƒ‡ãƒãƒƒã‚°ç”¨ãƒ•ãƒ©ã‚°ã‚’è¨­å®šã™ã‚‹
     *
     * @param debugMode ãƒ‡ãƒãƒƒã‚°ãƒ¢ãƒ¼ãƒ‰ã®æœ‰åŠ¹ãƒ»ç„¡åŠ¹
     */
...
import * as $ from './cubismmotion.js';
